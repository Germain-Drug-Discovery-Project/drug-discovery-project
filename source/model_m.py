...
class Modeling_class():
    ...
    def regression_modeling(self, metric_types = ['neg_root_mean_squared_error', 'r2'], splits = 10, scaler_type = None, grid_search = True):
        ...
        # Preforming X/y split on data and target
        X_train, X_test, y_train, y_test = self.split(self.scaled_df, target)

        model_scores = {'model_type': [], 'metric': [], 'scores': []}
        avg_scores = {'model_type': [], 'metric': [], 'avg_score': []}
        
        outputs = [] # init empty results list
        for (model_type, name) in self.models: # iterate through zipped models
            model_spec_out = [] # empty list init each iteration for use model specific iterables
            ...
            for metric in metric_types:
                metric_score = [abs(np.round(out[f'test_{metric}'], 3)) for out in model_spec_out] # results of validation scores for this metric
                avg_result = [abs(np.round(out[f'test_{metric}'], 3)).mean() for out in model_spec_out] # list comp to get mean of cross val tests for this metric
                
                avg_scores['model_type'].append(model_type)
                avg_scores['metric'].append(metric)
                avg_scores['avg_score'].append(avg_result)

                model_scores['model_type'].append(model_type)
                model_scores['metric'].append(metric)
                model_scores['scores'].append(metric_score)
        ...
        print('Modeling done!')
        return avg_scores, model_scores, outputs
    ...
    def classification_modeling(self, metric_type = 'accuracy', splits = 3):
        ...
        try: # Checking if scaling has already run, if yes there will be an attribute scaled df
            ...
            print('All done! Moving on to modeling, this may take a while...')
        target = (self.df.pIC50) # Setting target to pIC50
